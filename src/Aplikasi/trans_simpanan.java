...
private void awal()
 ...
 bhapus.setEnabled(false);
    ...
    {
        con.setkoneksi();
        con.rs=con.st.executeQuery("select trans_simpanan.nosimpan,detail_simpanan.no_ang,debet,kredit,detail_simpanan.saldo,tglsimpan,jenis,nama_ang,alamat,pekerjaan\n" +
"from detail_simpanan left join trans_simpanan on trans_simpanan.nosimpan = detail_simpanan.nosimpan\n" +
"left join anggota on anggota.no_ang = detail_simpanan.no_ang\n" +
"where trans_simpanan.nosimpan like '%"+cari+"%'");
        if (con.rs.next())
        {
            tnosim.setText(con.rs.getString(1));       ttgl.setText(con.rs.getString(6));
            tnoang.setText(con.rs.getString(2));       tket.setText(con.rs.getString(7));
            tsaldo.setText(con.rs.getString(5));       tnama.setText(con.rs.getString(8));
                                                       talamat.setText(con.rs.getString(9));
                                                       tpekerjaan.setText(con.rs.getString(10));
    ...
    private void tnoangKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tnoangKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_ENTER)
        {
            ...
            {
                con.setkoneksi();
                con.rs=con.st.executeQuery("select saldo from detail_simpanan where no_ang='"+tnoang.getText()+"'");
                if (con.rs.next())
                {
                    tlama.setText(con.rs.getString(1));
                    tdebet.requestFocus();
                }
                else 
                {
                    con.rs2=con.st.executeQuery("select saldo from simpan where no_ang='"+tnoang.getText()+"'");
                    if (con.rs2.next())
                {
                    tlama.setText(con.rs2.getString(1));
                    tdebet.requestFocus();
                } else{
                    tlama.setText("0");
                    }
    ...
    try
        {
            ...
            {
                String sql="update trans_simpanan set keterangan='"+tket.getText()+"',saldo='"+tsaldo.getText()+"' where no_ang='"+tnoang.getText()+"'";
                con.st.executeUpdate(sql);
                String sql1="update detail_simpanan set no_ang='"+tnoang.getText()+"',debet='"+tdebet.getText()+"',kredit='"+tkredit.getText()+"',saldo='"+tsaldo.getText()+"' where no_ang='"+tnoang.getText()+"'";
                con.st.executeUpdate(sql1);
            }
            awal();
            aktif();
